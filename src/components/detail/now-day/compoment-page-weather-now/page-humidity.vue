<template>
  <div class="w-full h-full">
    ...
    >
      <div class="flex items-center gap-2">
        <img
          width="24"
          height="24"
          src="../../../../assets/images/svg/ic_humidity.svg"
          alt=""
        />

        <p class="txt_bold txt-v2">
          {{
            $t("Humidity_in_the_coming_hours_in_{name}", {
    ...
    <!--  -->
    <div
      v-if="objectCurrently && objectCurrently.time"
      class="w-full h-[40px] flex justify-between items-center gap-2 p-2"
    >
    ...
    <div v-else class="w-full h-[40px] p-2 gap-2">
...
export default {
  ...
  mounted() {
    if (this.listHourly) {
      this.handlerHumidity(this.listHourly);
    }
  },
  methods: {
    ...
    generateSeriesData(data) {
      return data.map((item) => Math.round(item.humidity * 100) || 0);
    },
